...
public:
	...
	code_path[256],
	log_path[256];
	static PQMultiQuery * worker;
	static float * data;
	static int d, w, T, N, kc, m, M;
};
	...
	for(int i = 0; i < 3; i++) {
		...
		for(int j = 0; j < M; j++) {
			...
			worker->search_multi2(
					...
					sum,R,w,T,m,e,t1,t2,false,false);
			ed = clock();
	...
	for(int i = 0; i < 3; i++) {
		...
		for(int j = 0; j < N; j++) {
			...
			worker->search_multi2(
					...
					s1,s2,
					prebuck,cache,traversed,
					sum,R,w,T,m,e,t1,t2,true,false);
			ed = clock();
			t += static_cast<double>(ed - st);
			EXPECT_FALSE(traversed[0]);
			for(int i = 0; i < (R>sum?sum:R); i++)
